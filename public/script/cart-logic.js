...
function renderCart() {
    ...
    list.innerHTML = cart.map((item, index) => `
        <div class="cart-item">
            <div class="cart-img-box"><img src="${item.IMGURL}" alt="${item.Name}"></div>
            <div class="flex-grow-1">
                <span class="text-primary small fw-bold text-uppercase">${item.Brand}</span>
                <h5 class="text-white m-0">${item.Name}</h5>
                <p class="text-light m-0" style="opacity: 0.7;">${item.Price.toLocaleString()} Ft / db</p>
            </div>
            ...
            <div class="fw-bold text-white" style="min-width: 100px; text-align: right;">${(item.Price * item.quantity).toLocaleString()} Ft</div>
            <div class="remove-item" onclick="deleteItem(${index})"><i class="fa-solid fa-xmark"></i></div>
        </div>
    `).join('');
    calculateSummary();
}
...
function changeQty(index, delta) {
    ...
    if (delta > 0) {
        if (item.quantity >= item.maxStock) {
            alert(`Sajnos nincs több készleten! (${item.maxStock} db)`);
            return;
        }
        item.quantity += 1;
    } else {
...
async function checkout() {
    ...
    if (!currentAddressId) return alert("Rögzítse szállítási címét!");
    const cardName = document.getElementById('cardName').value.trim();
    const cardNumber = document.getElementById('cardNumber').value.trim();
    const cardExpiry = document.getElementById('cardExpiry').value.trim();
    const cardCvv = document.getElementById('cardCvv').value.trim();
    if (!cardName || !cardNumber || !cardExpiry || !cardCvv) return alert("Adja meg a kártyaadatokat!");
    try {
