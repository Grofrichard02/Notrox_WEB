...
(function injectBaseStyles() {
    ...
    style.textContent = `
        ...
        /* Navigációs gombok alapértelmezett rejtése a felvillanás ellen */
        #navButtons, #LoggedInNavButtons {
            display: none; /* Alapból egyik sem látszik */
            list-style: none;
        ...
        /* Gombok és linkek közös stílusa - border nélkül, kért háttérrel */
...
async function uploadProfilePic(event) {
    ...
    if (!allowedExtensions.exec(fileName) || !allowedMimeTypes.includes(file.type)) {
        alert("Hiba: Csak .jpg, .jpeg vagy .png képeket tölthetsz fel!");
        event.target.value = ""; 
        return;
    }
...
async function userRegister() {
    ...
    const pass = document.getElementById('password').value;
    const passag = document.getElementById('passwordag').value;
...
function openDeleteProfileModal() {
    document.getElementById('deleteProfileInput').value = "";
    document.getElementById('confirmDeleteProfileBtn').disabled = true;
...
const deleteInput = document.getElementById('deleteProfileInput');
...
if (confirmDeleteBtn) {
    confirmDeleteBtn.onclick = async function() {
        ...
        if (!uid) return alert("Hiba: Felhasználó nem azonosítható!");
